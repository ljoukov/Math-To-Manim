...
- **Scene 5 - QED Lagrangian Heart**: Presenting the complete QED Lagrangian density $`\mathcal{L}_{\text{QED}} = \bar{\psi}(i\gamma^\mu D_\mu - m)\psi - \frac{1}{4}F_{\mu\nu}F^{\mu\nu}`$ with color-coded term-by-term breakdown: Dirac spinor $`\psi`$ (fermion fields), gamma matrices $`\gamma^\mu`$ (Dirac algebra), covariant derivative $`D_\mu`$ (gauge coupling), mass term $`m`$, and field strength $`F_{\mu\nu}`$ (photon dynamics)
